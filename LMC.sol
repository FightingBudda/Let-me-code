...
contract LMCToken is Context, IERC20, Ownable {
  ...
  */
  ...
  function mint(address account,uint256 amount) public onlyOwner returns (bool) {
    //Level UP
    uint256 _mylevel = _level[account];
    _level[account] = _mylevel.add(1);
    emit LevelUp(account,   _level[account] );

    //Get Token
    uint256 _amount = amount * _base * 1e18;
